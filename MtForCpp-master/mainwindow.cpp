    ...
    ui(new Ui::MainWindow)
{
    ...
    pixmap_items = new QGraphicsPixmapItem*[X * Y];//pixmap_items 是二级指针
...
void MainWindow::DisplayFloor(int floor)
{
    ...
    for(y = 0; y <= Y - 1; y++){
        for(x = 0; x <= X - 1; x++){
            ...
            {
                pixmap_items[item_it] = new QGraphicsPixmapItem; //给一级指针分配内存
...
void MainWindow::OnFightTimerTriggered()//战斗界面
{
    ...
    }else if (fight_period_it % 2 == 1 && fight_end_it == 0)
    {
        ...
        {
            ui->label_20->setText(QString::number(0));//ui->label_20 金币
            fight_end_it = 1;
        }else{
...
void MainWindow::OnGainItemTimerTriggered()//显示获得的物品信息是通过Label控件显示的
{
    if (gain_item_it == 0)
    {
